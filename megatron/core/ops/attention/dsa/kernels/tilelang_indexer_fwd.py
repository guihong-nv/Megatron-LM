...
def _get_clean_logits_kernel(threads: int = 512, block_K: int = 4096):
    return _get_cached_kernel(
        ...
        _tilelang_indexer_fwd_cache_lock,
        (threads, block_K),
        lambda: clean_logits_(threads=threads, block_K=block_K),
    )
...
def _get_indexer_fwd_kernel(
    ...
    return _get_cached_kernel(
        _tilelang_indexer_fwd_kernel_cache,
        _tilelang_indexer_fwd_cache_lock,
        (heads, index_dim, block_N, num_stages, threads, use_relu),
        lambda: tl_indexer_fwd_impl(
            heads=heads,
            index_dim=index_dim,
            block_N=block_N,
...
_TL_INDEXER_FWD_PASS_CONFIGS = (
    {tilelang.PassConfigKey.TL_ENABLE_FAST_MATH: True} if HAVE_TILELANG else {}
)


@tilelang_jit(pass_configs=_TL_INDEXER_FWD_PASS_CONFIGS)
def tl_indexer_fwd_impl(  # pragma: no cover
    heads, index_dim, block_N=256, num_stages=3, threads=512, block_Q=None, use_relu=True
):
            ...
            for nbn_i in T.Pipelined(
                ...
                for bn_i, bq_i, h_i in T.Parallel(block_N, block_Q, heads):
                    s_reshaped[bn_i, bq_i, h_i] = (
                        T.max(s_reshaped[bn_i, bq_i, h_i], 0)
                        if use_relu
                        else s_reshaped[bn_i, bq_i, h_i]
                    ) * weights[bq_i, h_i]
...
@tilelang_jit
def clean_logits_(threads: int = 512, block_K: int = 4096):  # pragma: no cover
...
def indexer_fwd_interface(  # pragma: no cover
    q, kv, weights, cu_seqlen_ks, cu_seqlen_ke, clean_logits=True, use_relu=True
):
    """Run indexer forward kernel and optionally clean logits by row bounds."""
    require_tilelang()
    seq_len, heads, index_dim = q.shape
    seq_len_kv = kv.shape[0]
    weights = weights.to(dtype=torch.float32).contiguous()

    tl_indexer_fwd_kernel = _get_indexer_fwd_kernel(
        heads=heads, index_dim=index_dim, use_relu=use_relu
    )
